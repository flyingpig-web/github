/* =========================================================================
   bohun-AR4 공통 스크립트 (bohun-AR1 에서 확립한 공통 자산)
...
(function (global) {
  ...
  const Sound = (() => {
    ...
    /* 배경음 예약 재생 — 브라우저 자동재생 차단 대비.
       즉시 시도하고, 막히면 첫 사용자 제스처(포인터/키)에서 한 번 더 시도한다. */
    function armBgm(src, opts) {
      playBgm(src, opts);
      const kick = () => {
        try {
    ...
    /* 내레이션(V.O) — bgm/sfx 와 독립된 1채널.
       ...
       재사용: AR.Sound.playNarration("audio/intro_1-2.wav") */
    let voice = null;
  ...
  function CutRunner(config) {
    ...
    function crossTo(imgUrl) {
      ...
      // 자막/핫스팟/버튼보다 아래로 유지하면서 두 bg 레이어 간 위/아래만 제어
      nxt.style.zIndex = "1";
      cur.style.zIndex = "0";
